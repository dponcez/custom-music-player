...
export const handleDarkMode = (dark, active, [dataset, main, mode, inactive]) => {
  const htmlRefs = {
  ...
  eventHandler(toggleDarkMode, 'click', () => {
    ...
    inactiveElement.forEach(inactive => inactive.classList.toggle(`${active}`))
  ...
  const darkMode = (key, value) => {
    ...
    if(localTheme === value){
      ...
      inactiveElement.forEach(inactive => inactive.classList.add(`${active}`))
    }else{
      container.classList.remove(`${dark}`);

      modeElement.forEach(mode => mode.classList.remove(`${dark}`));
      inactiveElement.forEach(inactive => inactive.classList.remove(`${active}`))
    }
  }
